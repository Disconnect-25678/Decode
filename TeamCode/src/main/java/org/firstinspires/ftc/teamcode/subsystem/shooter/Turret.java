...
public class Turret extends SubsystemBase {
    ...
    public static double turretEncoderOffset = 0.943;
    ...
    public void recalibrateTurret() {
        ...
        double deg = getAbsolutePosition();

        if (deg > maxAngle) deg -= 360;
        else if (RobotMemory.turretPosition < 0) deg -= 360;

        RobotMemory.turretPosition = 0;

        this.positionOffset = angleToMotorPosition(deg);
    ...
    private double getAbsolutePosition() {
        return (360 -
                TurretInterpolatingTable.get(
                        (encoder.getVoltage() - turretEncoderOffset + 5) % 5
                )
        + 360) % 360;
    }
    ...
    public void periodic() {
        ...
        double power = controller.calculate(motorPosition, targetPosition);
        power = power + Math.signum(power) * kS;
        ...
        telemetry.addData("Turret relative angle: ", relativeAngle);
